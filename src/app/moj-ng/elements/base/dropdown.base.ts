import { Input, OnInit, ElementRef, Injector, SkipSelf, Host, Optional, TemplateRef, ContentChildren, QueryList, AfterContentInit } from '@angular/core';
...
export abstract class DropDownBase extends ElementBase<any> implements ControlValueAccessor, OnInit, AfterContentInit {
  ...
  @Input() placeholder: string = 'MojTexts.Choose';
      ...
      if (this.items && this.items.length > 0 && this.isIdModel) {
        ...
        if (!this.multiple) {
          ...
          let elementPos = this.items
            ...
            .indexOf(value);
          let selectedItem = elementPos >= 0 ? this.items[elementPos] : null;
