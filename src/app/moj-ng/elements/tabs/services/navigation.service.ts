...
import { ActivatedRouteSnapshot, CanActivate, Event as NavigationEvent, NavigationEnd, Router, RouterStateSnapshot } from '@angular/router';
  ...
  constructor(private router: Router) {
    this.routerEvents = router.events.subscribe((event: NavigationEvent) => {
      if (event instanceof NavigationEnd) { // After Navigation
        ...
        setTimeout(() => {         
          this.handleNavigatedUrlSubject.next( event.url == "/" ? "" : event.url);
        }, WAITE_TAB_LOAD);
      }
    });
  }
