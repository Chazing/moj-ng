...
import { MojFileUploadDesignType, MojBaseFileUploadComponent, FileUpload, fuAnimations } from '../moj-file-upload.base';
...
/**
  * Usage example
  ```html
    <moj-file-upload name="docsForApprove" [labelTextKey]="'Texts.docsForApprove'" [enabledFileTypes]="'pdf|png|jpg'" [(ngModel)]="files" [controlWidthColumns]="6"
			[required]="isRequired1" (fileUploadComplete)="fileUploadComplete($event)"></moj-file-upload>
	<moj-file-upload name="docsForCheck" [labelTextKey]="'Texts.docsForCheck'" [enabledFileTypes]="'pdf'" [(ngModel)]="files2" [controlWidthColumns]="6"
            [isWrapped]="true"></moj-file-upload>
    <moj-file-upload name="entityFiles" [labelTextKey]="'Texts.fileToUpload'" [enabledFileTypes]="'pdf|png|jpg'"
...
export class MojFileUploadComponent extends MojBaseFileUploadComponent {
    ...
    loadChunk(start, end, upload, chunkNum, successfulRequests, failedRequests, completedChunks, chunksQuantity, BYTES_PER_CHUNK) {
        ...
        // this.http.request(req).subscribe(d=>{
        //     debugger;
        // })
